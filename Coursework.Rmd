...
1. First we will remove columns 1 to 5 as these contain values that would not make sense as predictors such as row id, username and timestamps
...
# Remove columns 1-5 which do not make sensible predictors in training set
training.first5columnsremoved <- training.raw[ -c(1:5) ]
# Remove columns containing NA in training set
training.first5columnsandnasremoved <- training.first5columnsremoved[, which(colMeans(!is.na(training.first5columnsremoved)) == 1)]
# Remove columns 1-5 which do not make sensible predictors in testing set
testing.first5columnsremoved <- testing.raw[ -c(1:5) ]
# Remove columns containing NA in testing set
testing.first5columnsandnasremoved <- testing.first5columnsremoved[, which(colMeans(!is.na(training.first5columnsremoved)) == 1)]
inTrain = createDataPartition(y = training.first5columnsandnasremoved$classe, p = 0.8, list=FALSE)
training = training.first5columnsandnasremoved[ inTrain,]
testing = training.first5columnsandnasremoved[-inTrain,]
set.seed(123)
...
holdout<-predict(preProc,testing.first5columnsandnasremoved)
preProc
```
...
Next we will try a random forest model using 10 fold cross-validation, which based on having seen data scientists that I work with using AutoML, usually seems to come out near or at the top of the model scoring leaderboard (along with gradient boosted trees)
...
Next we will try a gradient boosted model using 10 fold cross-validation, which based on having seen data scientists that I work with using AutoML, usually seems to come out near or at the top of the model scoring leaderboard (along with random forest)
